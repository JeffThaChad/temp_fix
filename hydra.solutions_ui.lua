--[[
    ui library made for pie.solutions
]]
...
do
    ...
    library = {
        ...
        opening = false;
        hasInit = false;
        cheatname = startupArgs.cheatname or "hydra.solutions new ui lol";
    ...
    theme = {
        -- 17, 52, 22
        ["Accent"]                    = fromrgb(57, 131, 223),
        ["Background"]                = fromrgb(16, 14, 20),
        ["Border"]                    = fromrgb(0,0,0),
        ["Border 1"]                  = fromrgb(36, 39, 45),
        ["Border 2"]                  = fromrgb(37, 35, 42),
        ["Border 3"]                  = fromrgb(0,0,0),
        ["Primary Text"]              = fromrgb(220,220,220),
        ["Group Background"]          = fromrgb(19, 20, 27),
        ["Selected Tab Background"]   = fromrgb(20, 18, 24),
        ["Unselected Tab Background"] = fromrgb(13, 11, 17),
        ["Selected Tab Text"]         = fromrgb(57, 131, 223),
        ["Unselected Tab Text"]       = fromrgb(190,190,190),
        ["Section Background"]        = fromrgb(20, 18, 24);
        ...
        ["Option Border 2"]           = fromrgb(36, 39, 45);
        ["Option Background"]         = fromrgb(30, 28, 32);
...

{
            ...
            function window:AddTab(text, order)
                ...
                function tab:AddSection(text, side, order)
                    ...
                    do
                        ...
                        objs.topBorder2.Size += newUDim2(0,0,0,2)
    ...
    function library:CreateSettingsTab(menu)
        ...
        local settings = menu:AddTab("settings", 999); do

            local settings_main_section = settings:AddSection("main", 1); do

                settings_main_section:AddBind({text = "open / close", flag = "togglebind", nomouse = true, noindicator = true, bind = Enum.KeyCode.LeftAlt, callback = function()
                    library:SetOpen(not library.open)
                end})

                settings_main_section:AddToggle({text = "disable movement if open", flag = "disablemenumovement", callback = function(bool)
                    local context_action_service = game["ContextActionService"]
                ...
                settings_main_section:AddBind({text = "unload key", flag = "unload_key", nomouse = true, noindicator = true, bind = Enum.KeyCode.RightShift, callback = function()
                    library:Unload()
                end})

                settings_main_section:AddButton({text = "unload", confirm = true, risky = true,  callback = function()
                    library:Unload()
                end})

                settings_main_section:AddButton({text = "copy server connect", callback = function()
                    setclipboard(([[game:GetService("TeleportService"):TeleportToPlaceInstance(%s, "%s")]]):format(game.PlaceId, game.JobId));
                end});
                settings_main_section:AddButton({text = "Rejoin Game", confirm = true, callback = function()
            ...
            -- Configurations
            local settings_config_section = settings:AddSection("configuration", 2); do
                ...
                settings_config_section:AddBox({text = "config name", flag = "configinput"})
                settings_config_section:AddList({text = "config", flag = "selected_config"})
                settings_config_section:AddButton({text = "load", confirm = true, callback = function()
                    library:LoadConfig(flags.selected_config);
                end});
                settings_config_section:AddButton({text = "save", confirm = true, callback = function()
                    library:SaveConfig(flags.selected_config);
                end});
                settings_config_section:AddButton({text = "create", confirm = true, callback = function()
                ...
                settings_config_section:AddButton({text = "delete", confirm = true, callback = function()
                ...
                settings_config_section:AddButton({text = "refresh", callback = function()
                    refresh_configs();
                end});
                refresh_configs();
...
return library
--[[library:init()
local window = library.NewWindow({title = "hydra.solutions", size = UDim2.new(0, 625, 0, 808)});
